...
# !! This haversine is generated by deepseek, not verified yet.
...
# !! This detect_duplicates is generated by deepseek, not verified yet.
def detect_duplicates(
    location_data: list[tuple[LocationSummary, str, str]],
) -> dict[str, list[tuple[LocationSummary, str, str]]]:
    ...
    for idx, (loc, pid, pname) in enumerate(location_data):
        if loc.point_y_wgs84_web and loc.point_x_wgs84_web:
            grid_key = (
                round(loc.point_y_wgs84_web / GRID_SIZE),
            ...
            for dy in [-1, 0, 1]:
                ...
                for neighbor_idx in grid.get(neighbor_grid, []):
                    ...
                    distance = haversine(
                        base_lat,
                        base_lon,
                        nloc.point_y_wgs84_web,
                        nloc.point_x_wgs84_web,
                    )
                    ...
                    if distance <= 50:
                        ...
                        loc_method_names = sorted(
                            [method.name for method in loc.methods or []]
                        )
                        nloc_method_names = sorted(
                            [method.name for method in nloc.methods or []]
                        )
